...
class Body:
    """
    A generic class for a pybullet body.
    """
    ...
    def __init__(self,
        ...
        :param body: a body object from pybullet (p.loadURDF)
        :param env: The environment in which the body exists
        :param controllable_joints:
        ...
        self.body = body  # The pybullet body object id
        self.env_id = env.id
        self.env = env
        self.all_joints = list(range(env.sim.getNumJoints(body)))
    ...
    def control(self, targets, joints=None, gains=None, forces=None, velocity_control=False, set_instantly=False):
        ...
        if not velocity_control:
            p.setJointMotorControlArray(self.body, jointIndices=joints, controlMode=p.POSITION_CONTROL,
                                        targetPositions=targets, positionGains=gains, forces=forces,
                                        physicsClientId=self.env_id)
        else:
            p.setJointMotorControlArray(self.body, jointIndices=joints, controlMode=p.VELOCITY_CONTROL,
                                        targetVelocities=targets, velocityGains=gains, forces=forces,
                                        physicsClientId=self.env_id)
        if set_instantly:
    ...
    def get_joint_angles(self, joints=None, include_fixed_joints=True):
        ...
        if not include_fixed_joints:
            ...
            robot_joint_states = p.getJointStates(self.body, jointIndices=joints, physicsClientId=self.env_id)
    ...
    def get_link_pos_orient(self, link, center_of_mass=False, local_coordinate_frame=False):
        ...
        if link == self.base:
            pos, orient = p.getBasePositionAndOrientation(self.body, physicsClientId=self.env_id)
        else:
            if not center_of_mass:
                pos, orient = p.getLinkState(self.body, link, computeForwardKinematics=True,
                                             physicsClientId=self.env_id)[
                              4:6]
            else:
                pos, orient = p.getLinkState(self.body, link, computeForwardKinematics=True,
    ...
    def global_to_local_coordinate_frame(self, pos, orient=[0, 0, 0, 1], link=-1, rotation_only=False):
        ...
        link_pos_inv, link_orient_inv = p.invertTransform(link_pos, link_orient, physicsClientId=self.env_id)
        real_pos, real_orient = p.multiplyTransforms(link_pos_inv if not rotation_only else [0, 0, 0], link_orient_inv,
                                                     pos, self.get_quaternion(orient), physicsClientId=self.env_id)
    ...
    def local_to_global_coordinate_frame(self, pos, orient=[0, 0, 0, 1], link=-1, rotation_only=False):
        link_pos, link_orient = self.get_base_pos_orient() if link == self.base else self.get_link_pos_orient(link)
        real_pos, real_orient = p.multiplyTransforms(link_pos if not rotation_only else [0, 0, 0], link_orient, pos,
                                                     self.get_quaternion(orient), physicsClientId=self.env_id)
        return np.array(real_pos), np.array(real_orient)
    ...
    def get_link_velocity(self, link):
        if link == self.base:
            return np.array(p.getBaseVelocity(self.body, physicsClientId=self.env_id)[0])
        return np.array(p.getLinkState(self.body, link, computeForwardKinematics=True, computeLinkVelocity=True,
                                       physicsClientId=self.env_id)[6])

    def get_base_linear_velocity(self):
        return np.array(p.getBaseVelocity(self.body, physicsClientId=self.env_id)[0])

    def get_base_angular_velocity(self):
        return np.array(p.getBaseVelocity(self.body, physicsClientId=self.env_id)[1])

    def get_euler(self, quaternion):
        return np.array(quaternion) if len(quaternion) == 3 else np.array(
            p.getEulerFromQuaternion(np.array(quaternion), physicsClientId=self.env_id))

    def get_quaternion(self, euler):
        return np.array(euler) if len(euler) == 4 else np.array(
            p.getQuaternionFromEuler(np.array(euler), physicsClientId=self.env_id))

    def get_link_mass(self, link):
        return p.getDynamicsInfo(self.body, link, physicsClientId=self.env_id)[0]

    def get_motor_joint_states(self, joints=None):
        # Get the position, velocity, and torque for nonfixed joint motors
        joint_states = p.getJointStates(self.body, self.all_joints if joints is None else joints,
                                        physicsClientId=self.env_id)
        joint_infos = [p.getJointInfo(self.body, i, physicsClientId=self.env_id) for i in
                       (self.all_joints if joints is None else joints)]
    ...
    def get_joint_max_force(self, joints=None):
        if joints is None:
            joints = self.all_joints
        joint_infos = [p.getJointInfo(self.body, i, physicsClientId=self.env_id) for i in joints]
    ...
    def get_contact_points(self, bodyB=None, linkA=None, linkB=None, average=False):
        args = dict(bodyA=self.body, physicsClientId=self.env_id)
        ...
        if average and bodyB is not None:
            ...
            return dict(bodyA=cp[0][1], bodyB=cp[0][2], linkA=cp[0][3], linkB=cp[0][4], posA=avg(5), posB=avg(6),
                        contact_normal=avg(7), contact_distance=avg(8), normal_force=avg(9, True),
                        lateral_friction_1=avg(10, True), lateral_friction_dir_1=avg(11),
                        lateral_friction_2=avg(12, True), lateral_friction_dir_2=avg(13))
        return [dict(bodyA=c[1], bodyB=c[2], linkA=c[3], linkB=c[4], posA=c[5], posB=c[6], contact_normal=c[7],
                     contact_distance=c[8], normal_force=abs(c[9]), lateral_friction_1=abs(c[10]),
                     lateral_friction_dir_1=c[11], lateral_friction_2=abs(c[12]), lateral_friction_dir_2=c[13]) for c in
                cp]
    ...
    def get_closest_points(self, bodyB, distance=4.0, linkA=None, linkB=None):
        args = dict(bodyA=self.body, bodyB=bodyB.body, distance=distance, physicsClientId=self.env_id)
        if linkA is not None:
            args['linkIndexA'] = linkA
        if linkB is not None:
            args['linkIndexB'] = linkB
    ...
    def get_heights(self, set_on_ground=False):
        ...
        for i in self.all_joints + [self.base]:
            min_pos, max_pos = p.getAABB(self.body, i, physicsClientId=self.env_id)
    ...
    def get_AABB(self, link=-1):
        min_pos, max_pos = p.getAABB(self.body, link, physicsClientId=self.env_id)
        return min_pos, max_pos

    def get_force_torque_sensor(self, joint):
        return np.array(p.getJointState(self.body, joint, physicsClientId=self.env_id)[2])
    ...
    def get_jacobian(self, link, target_joint_accelerations=None):
        ...
        target_joint_accelerations = [0] * len(joint_positions) if target_joint_accelerations is None else list(
            target_joint_accelerations)

        linear_jacobian, angular_jacobian = p.calculateJacobian(self.body, link, localPosition=[0] * 3,
                                                                objPositions=joint_positions,
                                                                objVelocities=joint_velocities,
                                                                objAccelerations=target_joint_accelerations,
                                                                physicsClientId=self.env_id)
    ...
    def set_base_pos_orient(self, pos=None, orient=None):
        ...
        p.resetBasePositionAndOrientation(self.body, pos, self.get_quaternion(orient), physicsClientId=self.env_id)

    def set_base_velocity(self, linear_velocity, angular_velocity=[0, 0, 0]):
        p.resetBaseVelocity(self.body, linearVelocity=linear_velocity, angularVelocity=angular_velocity,
                            physicsClientId=self.env_id)
    ...
    def set_joint_angles(self,
        ...
        for i, (j, a) in enumerate(zip(joints, angles)):
            self.env.sim.resetJointState(self.body, jointIndex=j,
                                         targetValue=min(max(a, self.lower_limits[j]),
                                                         self.upper_limits[j]) if use_limits else a,
    ...
    def set_frictions(self, links, lateral_friction=None, spinning_friction=None, rolling_friction=None):
        ...
        for link in links:
            if lateral_friction is not None:
                p.changeDynamics(self.body, link, lateralFriction=lateral_friction, physicsClientId=self.env_id)
            if spinning_friction is not None:
                p.changeDynamics(self.body, link, spinningFriction=spinning_friction, physicsClientId=self.env_id)
            if rolling_friction is not None:
                p.changeDynamics(self.body, link, rollingFriction=rolling_friction, physicsClientId=self.env_id)
    ...
    def set_mass(self, link, mass):
        p.changeDynamics(self.body, link, mass=mass, physicsClientId=self.env_id)

    def set_restitution(self, restitution):
        for link in self.all_joints + [self.base]:
            p.changeDynamics(self.body, link, restitution=restitution, physicsClientId=self.env_id)
    ...
    def set_joint_stiffness(self, joint, stiffness):
        p.changeDynamics(self.body, joint, jointDamping=stiffness, physicsClientId=self.env_id)
        # p.changeDynamics(self.body, joint, contactStiffness=stiffness, contactDamping=stiffness, physicsClientId=self.env_id)

    # def set_gravity(self, ax=0.0, ay=0.0, az=-9.81):
    #     p.setGravity(ax, ay, az, body=self.body, physicsClientId=self.env_id)

    def enable_force_torque_sensor(self, joint):
        p.enableJointForceTorqueSensor(self.body, joint, enableSensor=True, physicsClientId=self.env_id)

    def apply_external_force(self, link=-1, force=[0, 0, 0], pos=[0, 0, 0], local_coordinate_frame=False):
        p.applyExternalForce(self.body, link, force, pos, p.LINK_FRAME if local_coordinate_frame else p.WORLD_FRAME,
                             physicsClientId=self.env_id)

    def apply_external_torque(self, link=-1, torque=[0, 0, 0], pos=[0, 0, 0], local_coordinate_frame=False):
        p.applyExternalTorque(self.body, link, torque, pos, p.LINK_FRAME if local_coordinate_frame else p.WORLD_FRAME,
                              physicsClientId=self.env_id)

    def create_constraint(self, parent_link, child, child_link, joint_type=p.JOINT_FIXED, joint_axis=[0, 0, 0],
                          parent_pos=[0, 0, 0], child_pos=[0, 0, 0], parent_orient=[0, 0, 0], child_orient=[0, 0, 0],
                          force=None):
        constraint = p.createConstraint(self.body, parent_link, child.body, child_link, joint_type, joint_axis,
                                        parent_pos, child_pos, self.get_quaternion(parent_orient),
                                        self.get_quaternion(child_orient), physicsClientId=self.env_id)
    ...
    def change_visual(self, link=-1, rgba=None, specular_color=None):
        if rgba is not None:
            p.changeVisualShape(self.body, link, rgbaColor=rgba, physicsClientId=self.env_id)
        if specular_color is not None:
            p.changeVisualShape(self.body, link, specularColor=specular_color, physicsClientId=self.env_id)
    ...
    def update_joint_limits(self, joints=None):
        ...
        self.ik_joint_names = []
        for j in joints:
            joint_info = p.getJointInfo(self.body, j, physicsClientId=self.env_id)
        ...
        for i in self.controllable_joints:
            ...
            for j in self.all_joints:
                ...
                joint_type = p.getJointInfo(self.body, j, physicsClientId=self.env_id)[2]
                if joint_type != p.JOINT_FIXED:
                    counter += 1
    ...
    def ik(self, target_joint, target_pos, target_orient=None, max_iterations=1000, use_current_joint_angles=False,
        ...
        if target_orient is not None:
            ik_joint_poses = np.array(p.calculateInverseKinematics(self.body, target_joint, targetPosition=target_pos,
                                                                   targetOrientation=self.get_quaternion(target_orient),
    ...
    def print_joint_info(self, show_fixed=True):
        joint_names = []
        for j in self.all_joints:
            info = p.getJointInfo(self.body, j, physicsClientId=self.env_id)
